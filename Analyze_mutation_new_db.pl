...
# Usage: $ perl Analyze_mutation.pl INPUT_Bowtie_file OUTPUT_file
...
### choose human or mouse

# 1) human files
open FILE_RNAfold, "<", "secondaryStructure.oneresult" or die $!; # RNAfold output file
open FILE_miRNA_Bowtie, "<", "hsa-hairpin-release21-subUtoT.againstHg19.fmt" or die $!; # miRNA aligned to genome
my $miRBase_string="hsa"; # type the miRBase string for the organism in question. For example, for human write "hsa" and for mouse write "mmu" (this data can be found in miRBase)   

# 2) mouse files
#open FILE_RNAfold, "<", "RNAfold_file_mouse.txt" or die $!; # RNAfold output file
#open FILE_miRNA_Bowtie, "<", "Mouse_pre_miRNA_aligned_against_the_genome.txt" or die $!; # miRNA aligned to genome
#my $miRBase_string="mmu"; # type the miRBase string for the organism in question. For example, for human write "hsa" and for mouse write "mmu" (this data can be found in miRBase)   
...
### general input files
open FILE_Bowtie_genome, "<", $file_input or die $!; # bowtie output file: aligning to genome using bowtie 
open FILE_mature, "<", "hsa-mature-release21.fa" or die $!; # mature miRNA file

### general output files
open FILE_out,">", $file_output or die $!; # The main output file
...
my $error_file = $file_output . "_log";
...
while ($string=<FILE_RNAfold>)
{
    if ($string =~ />(\S+)/)
    {
        ...
        if ($string =~ /(\S+)/)
        {
            ...
            if (defined $mature_miRNA_hash{$name_of_miRNA})
            {
                if (index($miRNA_seq_hash{$name_of_miRNA},$mature_miRNA_hash{$name_of_miRNA})!=(-1))
                {
					...
					####### hsa-mir-3142 and hsa-mir-4487 has two matched mature sequence position, only one is false, hardcode to chose the correct one ### wym
					if ($name_of_miRNA eq 'hsa-mir-3142' or $name_of_miRNA eq 'hsa-mir-4487') 
					{
						my $offset = $location_of_mature_miRNA + 1;
						$location_of_mature_miRNA = index($miRNA_seq_hash{$name_of_miRNA}, $mature_miRNA_hash{$name_of_miRNA}, $offset);
						#print "$name_of_miRNA\t$location_of_mature_miRNA\n";
					}
					###### hardcode end wym
				}
                else
                {
                    #print ERRORFILE "problem in mature miRNA: ",$name_of_miRNA,"\n"; # error output here all commented by wym, there is one check at last part of finding mature miRNA
                }
            }
            if (defined $mature_miRNA_hash{$name_of_miRNA."-5p"} or defined $mature_miRNA_hash{$name_of_miRNA."-3p"}) 
            {
			...
			#else # wym
			if ($length_of_mature_miRNA eq 0 or $length_of_mature_miRNA2 eq 0)
            {
                $temp_index='';
                $cut_name_of_miRNA='';
                $temp_index=rindex($name_of_miRNA,"-");
                $cut_name_of_miRNA=substr($name_of_miRNA,0,$temp_index);
                if (defined $mature_miRNA_hash{$cut_name_of_miRNA} and $name_of_miRNA ne "hsa-mir-3180-1" and $name_of_miRNA ne "hsa-mir-3180-2" and $name_of_miRNA ne "hsa-mir-3180-3")
                {
                ...
                elsif (defined $mature_miRNA_hash{$cut_name_of_miRNA."-5p"} or defined $mature_miRNA_hash{$cut_name_of_miRNA."-3p"})
                {

					######## hsa-mir-550b-1 has a mature named hsa-mir-550b-2-5p, hardcode to midify this unusual naming  # wym
					if ($name_of_miRNA eq 'hsa-mir-550b-1') {
						$altered_name_of_miRNA=$cut_name_of_miRNA."-2-5p";
						$location_of_mature_miRNA=index($miRNA_seq_hash{$name_of_miRNA},$mature_miRNA_hash{$altered_name_of_miRNA});
                        $length_of_mature_miRNA=length($mature_miRNA_hash{$altered_name_of_miRNA});
						#print "$name_of_miRNA\t$location_of_mature_miRNA\n"; # wymwym
					}
                ...
                {
                    #print ERRORFILE "can't find the mature miRNA: ",$name_of_miRNA,"\n";
                }
            }
			# wym
			if ($length_of_mature_miRNA eq 0 or $length_of_mature_miRNA2 eq 0)
            { # also possible that last digit is removed (e.g.hsa-mir-517a,hsa-mir-517b)
                $temp_index='';
                $cut_name_of_miRNA='';
                $cut_name_of_miRNA=substr($name_of_miRNA,0,length($name_of_miRNA)-1);
			...
			# check if all pre-miRNA have at least one mature miRNA
			if ($length_of_mature_miRNA eq 0 and $length_of_mature_miRNA2 eq 0) {
				print ERRORFILE "problem in pre-miRNA ",$name_of_miRNA," has no mature miRNA found \n";
			}
			
			# wym there is no star miRNA in miRbase release 21
            ...
            if (defined $star_miRNA_hash{$name_of_miRNA})
            {
                ...
                {
                    print ERRORFILE "problem in star miRNA: ",$name_of_miRNA,"\n";
                }                
            }            
            else
            {
			...
			# wym debug output
			#print $name_of_miRNA,"\t",$length_of_mature_miRNA,"\t",$length_of_mature_miRNA2,"\n";
			#if ($length_of_mature_miRNA eq 0 and $length_of_mature_miRNA2 eq 0) {
            #	print $name_of_miRNA," do not have any mature miRNA\n";
            #} else {
			#	print $name_of_miRNA," pass\n";
			#}
        ...
        {
            print ERRORFILE "can't read the miRNA sequence!\n";
        }
...
print ERRORFILE "end of pre-process part\n";
...
close FILE_Bowtie;
print ERRORFILE "done with temp_aligned_to_miRNA.txt","\n";
...
while ($string=<FILE_Bowtie>)
{
    ...
    {
        print ERRORFILE "read number=",$number_of_read,"\n";
    }
    # reading the data from the alignment file 
    @fields=();
    $name_of_miRNA='';
    ...
    $ref_bais=$fields[$location_of_ref_bais_field-1];
    $read_seq=$fields[$location_of_read_field-1];
    $read_seq =~ s/T/U/g;
    $length_of_read=length($read_seq);
    ...
    for ($i=$ref_bais;$i<($ref_bais+$length_of_read);$i++)
    {
...
print FILE_out "total number of reads:\t",$number_of_read,"\n";  # wym
...
for ($j=0;$j<=$#all_the_miRNA;$j++)
{
    ...
    if ($max_read>=$min_read_depth)
    {
		...
		if (($possible_end_of_mature-$possible_start_of_mature+1) eq 0) {$problem_total=$problem_total+1;$flag_problem=1;} #wym 
        elsif ((($cum_sum/($possible_end_of_mature-$possible_start_of_mature+1))<$length_cutoff_for_comparing_mature_miRNA)||($cum_sum_after_end>$max_miRBase_after_end))
        {
            #print FILE_out "problem in the miRbase mature definition in miRNA: ",$all_the_miRNA[$j],"\t",$possible_start_of_mature,":",$possible_end_of_mature,"\t",$cum_sum_after_end, "\t", $max_miRBase_after_end, "\t", $cum_sum, "\n";
...
print ERRORFILE "start analyze mismatches\n";
    ...
    foreach (@mismatch_types)
    {
        ...
        for ($j=0;$j<=$running_counter;$j++)
        {
            ...
            if (exists $hash_mismatch_vs_position{$i}{$_}[$j])
            {
                ...
                {
                    print ERRORFILE "something is wrong!\n";
                }
            }
        }
...
print ERRORFILE "printing output file\n";
