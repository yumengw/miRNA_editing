#!/usr/bin/perl -w
use strict;
use lib 'Math-CDF-0.1/lib64/perl5';
use Math::CDF qw(:all);
my $prob = pnorm(1.96);

### START OF INPUT ############################################################################################################################################
# In this step binomial statistics is performed on the output file (or files) of Analyze_mutation.pl to separate sequencing errors from statistically significant modifications.  
# Usage: $ perl Binomial_analysis.pl INPUT_file
#        OR
#        $ perl Binomial_analysis.pl INPUT_file1 INPUT_file2 INPUT_file3 ...
# Binomial_analysis.pl can receive several input files and merge them together

# Previous steps:
# 1) process_reads.pl
# 2) bowtie -n 1 -e 50 -a -m 1 --best --strata --phred33-quals --trim3 2 BOWTIE_INDEXES_PATHWAY FASTQ_FILE > BOWTIE_OUTPUT_FILE
# 3) Analyze_mutation.pl

my @file_names=@ARGV; # the output file or files of Analyze_mutation.pl
...
print "#CHROM\tSTART\tEND\tSTRAND\tmiRNA_name\tLocation_inside_pre_miRNA\tMismatch_type\tNumber_of_reads_with_the_mismatch\tTotal_number_of_reads_in_this_position\tRaw_P_value\tBonferroni_P_value\tBH_P_value\n";
foreach $key (sort { $hash_location_pvalue{$a} <=> $hash_location_pvalue{$b}} keys %hash_location_pvalue)
{
    if ($BH_or_Bonferroni eq "BH")
    {
		if (defined $BH_adjusted_p_values{$key}) { # wym debug prevent wrong output in BH mode
        	$multiple_tests_corrected_p_value=$BH_adjusted_p_values{$key};
		}
		else {
			$multiple_tests_corrected_p_value=1000; #wym random number, anything >=1 will work
		}
    }
    else
    {
    ...
    if ($multiple_tests_corrected_p_value<=$P_value)
    {
        ...
        {
            print $hash_convert_position_in_miRNA_to_genomic_position{$array_values[0]."\t".$array_values[1]},"\t",$key,"\t",$hash_location_pvalue{$key},"\t",$hash_location_pvalue{$key}*$length_p_values,"\t",$BH_adjusted_p_values{$key},"\n";
        }
    }
    else
    {
